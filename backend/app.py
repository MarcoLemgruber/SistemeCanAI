...
CAMERAS = {
    1: 0  # Webcam padrão do notebook/PC
}
active_camera = 1
...
def detect_objects(frame):
    ...
    for objectID, centroid in objects.items():
        ...
        # Entrou no ROI (esquerda -> direita)
        if prev is not None:
            if prev < line_x and centroid[0] >= line_x:
                total_in += 1
            # Saiu do ROI (direita -> esquerda)
            elif prev >= line_x and centroid[0] < line_x:
                total_out += 1
        track_history[objectID] = centroid[0]

    # Contar quantos IDs estão no ROI (direita)
    dentro = sum(1 for centroid in objects.values() if centroid[0] >= line_x)
    ...
    cv2.putText(frame, f'Dentro: {dentro}', (50, 200),
